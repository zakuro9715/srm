...
public:
  int theCount(vector<int> seq)
  {
    if(seq.size() == 1)
      return -1;
    int n = 2, d1 = seq[seq.size() - 1] + seq[1] - seq[0], d2 = seq[seq.size() - 1] * seq[1] / seq[0];
    ...
    else
    {
      for(int i = 1; i < seq.size(); i++)
      {
        if(seq[i] % seq[i - 1] || seq[i] / seq[i - 1] != seq[1] / seq[0])
        {
          n--;
          break;
        }
